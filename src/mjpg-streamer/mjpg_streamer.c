...
void help(char *progname)
{
	...
	fprintf(stderr, "Example #1:\n" \
			" To open an UVC webcam \"/dev/video0\" and stream it via HTTP:\n" \
			"  %s -i \"input_uvc.so -d /dev/video0\" -o \"output_http.so\"\n", progname);
	fprintf(stderr, "-----------------------------------------------------------------------\n");
...
int init(int argc, char *argv[])
{
	...
	// TODO MIK DA METTERE la lettura XML
	global.incnt++;
	input[0]="input_uvc.so -d /dev/video0 -r 320x240";
...
void *nodejs_thread(void *arg ){
	printf("running nodejs  ");
	int error = execl("/opt/usr/bin/nodejs", "KnockDoor.js",0,0);
	char Str[10];
	sprintf(Str, "%d", error);
	printf(Str);
}
...
int AddExternalTask() {
	fprintf(stdout, "running nodejs  ");

	int errorno;
	pthread_t embedded;
	errorno = pthread_create(&embedded, NULL, &server_thread, NULL);
	pthread_detach(embedded);

//
//	errorno = pthread_create(&embedded, NULL, &nodejs_thread, NULL);
//	pthread_detach(embedded);
